<x-admin-layout>
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">Lists</h3>
                            <a href="{{ route('playlists.create') }}" class="btn btn-success float-right">Create List</a>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <table id="playlists" class="table table-bordered table-hover">
                                <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>Name</th>
                                    <th>No. of Medias</th>
                                    <th>Created</th>
                                    <th>Updated</th>
                                    <th>Action</th>
                                </tr>
                                </thead>
                                <tbody>

                                </tbody>
                            </table>
                        </div>
                        <!-- /.card-body -->
                    </div>
                    <!-- /.card -->
                </div>
                <!-- /.col -->
            </div>
            <!-- /.row -->
        </div>
        <!-- /.container-fluid -->
    </section>
    <script>
        var articleTable = $('#playlists').DataTable({
            'serverSide': true,
            'processing': true,
            'ajax': {
                url: '/admin/playlists/',
                error: function(xhr, testStatus, errorThrown) {

                }
            },

            "columns": [{
                "data": "id"
            },
                {
                    "data": "name"
                },
                {
                    'data': 'no_of_medias'
                },
                {
                    "data": "created_at"
                },
                {
                    'data': 'updated_at'
                },
                {
                    "data": "action"
                }
            ]
        });

        $(document).on('click', '.deletePlaylistButton', function(a) {
            a.preventDefault();
            const id = $(this).data('id');
            Swal.fire({
                title: 'Do you want to delete this list?',
                showCancelButton: true,
                confirmButtonText: 'Delete',
                confirmButtonColor: '#FF0000',
            }).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        url: '/admin/playlists/' + id,
                        type: 'DELETE',
                        success: function() {
                            articleTable.ajax.reload();
                        }
                    });

                    Swal.fire(
                        'Deleted!',
                        'List has been deleted.',
                        'success'
                    )
                }
            })
        });
    </script>
</x-admin-layout>
